...
| `1.B.3-member-key-directory` | — | Member-Update-Workflow; macht `protocol/sync/space-capability.ts` (`createSpaceCapabilityJws` / `verifySpaceCapabilityJws`) produktiv konsumiert (heute nur in Tests). Behebt [#181](https://github.com/real-life-org/web-of-trust/issues/181): (a) member-update buffering nach Sync 005 §member-update — verworfene Messages bei fehlendem Group-Key puffern + nach Key-Import replayen (statt heute `return`); (b) `saveSpaceMetadata` dirty-check Fingerprint vervollständigt um `image`/`modules`/`appTag` + tatsächliche `memberEncryptionKeys`-Bytes (heute nur DIDs → stale Recipient-Keys möglich). | Sync 005 §member-update; Sync 003 §Capability-JWS |
