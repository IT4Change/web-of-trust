...
`packages/wot-core/src/crypto/` | `rewrite` | Protocol or remove | Duplicates protocol crypto concerns (`did`, `jws`, encoding, capabilities, envelope auth). Phase-1 Plan: **Verzeichnis wird komplett entfernt — keine Aliase, keine Re-Export-Shims** (siehe `migration/PHASE-1-WOT-CORE-DEMO.md#1a-querschnitt-konsolidierung`). Konsumenten werden auf `protocol/crypto`, `protocol/identity`, `protocol/sync` umgestellt.
`packages/wot-core/src/identity/` (removed) | n/a | n/a | Legacy `WotIdentity`/`SeedStorage` directory **wurde bereits entfernt**; identity flows laufen jetzt vollständig über `application/identity/identity-workflow.ts` und `IdentitySeedVault`. Eintrag bleibt als historische Notiz; keine offene Arbeit.
...
`packages/wot-core/src/services/AttestationDeliveryService.ts` (removed in 1.B.2) | n/a | n/a | Deleted in slice 1.B.2 as dead code (barrel-only consumers). Trust 001 Z.147 — `wot-trust@0.1` defines no `attestation-ack` semantic. Demo `AttestationService` retains Sync 003 transport delivery + retry + publish-consent (`accepted`); no Trust-level acceptance signal exists.
...
> **Status 2026-06-07**: 4 von 5 Items sind durch wot-spec-Änderungen (PR #13/#15/#22/#35/#48/#53/#56/#74) und Spec-Recheck spec-aufgelöst. Sie bleiben als historische Notiz erhalten, sind aber **nicht mehr blockierend**. Auflösungen siehe `migration/PHASE-1-WOT-CORE-DEMO.md#status-quo`.

- ~~Whether `packages/wot-core/src/crypto/capabilities.ts` and `envelope-auth.ts` should be migrated into `protocol/sync` or replaced by newer protocol modules needs an explicit compatibility decision.~~ **Resolved**: [wot-spec#95](https://github.com/real-life-org/wot-spec/issues/95) — capabilities are non-normative UCAN-style helpers, moved to `application/authorization/capabilities.ts` (slice 1.A.2). [wot-spec#96](https://github.com/real-life-org/wot-spec/issues/96) — envelope-auth contradicts Sync 003 (DIDComm-plaintext envelope + inner-JWS body), kept in `crypto/` with `@deprecated` marker until Phase 2+ Automerge-adapter-stack refactor. `src/crypto/` reduced to envelope-auth-only after 1.A.2.
