...
| In-person verification | User shows QR challenge, peer scans, confirms, sends verification, and both users see mutual verification state. | `wot-trust@0.1` | Verification challenge/response workflow plus Trust 002 raw QR creation, active challenge tracking, and volatile consumed nonce history. | Verification store, messaging/outbox send, discovery/profile sync, contact storage, optional broker configuration. |
...
| `apps/demo/src/services/AttestationService.ts:154`, `:203` + `apps/demo/src/hooks/useVerification.ts:159`, `:209`, `:272` | Legacy-`MessageEnvelope` mit `ref: createResourceRef('attestation', ...)` (5 Stellen) | Demo baut Sync-003-divergente Envelope-Form (`v: 1`, `fromDid`, `toDid`, `signature`, `ref`) statt der spec-konformen DIDComm-Plaintext-Form. SPEC-AUDIT.md B2ack-1, drift:blocker. | `1.B.2-verification`: neuer `application/verification/*`-Workflow liefert spec-konforme Envelopes; alle 5 Stellen hängen sich um. |
| `apps/demo/src/hooks/useProfileSync.ts:51` | Legacy-`MessageEnvelope` ohne `ref` für `profile-update`-Notification | Selbe Legacy-Form-Drift wie B2ack-1, andere Subkategorie. SPEC-AUDIT.md B2ack-2, drift:minor. | `1.D` Demo-Hooks-Migration auf Sync-003-konformen Envelope-Builder. |
...
> **Status 2026-06-08**: Die ursprünglichen 4 Items sind spec-aufgelöst (siehe Strikethrough). Ein neuer offener Punkt aus dem `SPEC-AUDIT.md`-Rebuild ist hinzugekommen.
...
- **Open (drift:offen, vor 1.B.2-verification klären)**: `DeliveryReceipt.status: 'accepted'` ist in `packages/wot-core/src/types/messaging.ts:49` als wot-core API-Surface definiert; Sync 003 §ack/1.0 definiert `accepted` aber nur als **Inbox-Acceptance-Signal** (Z.624: "bestaetigt nicht, dass ein Inhaltsartefakt semantisch angenommen wurde"). Mapping zwischen wot-core API und Sync-003-Wire-Form ist nicht explizit dokumentiert. SPEC-AUDIT.md B2ack-3. Klärung vor 1.B.2-verification-Start: ~30 Min Sync 003 Z.590ff lesen, Ergebnis als Spec-Anker-Block im PR-Body.
