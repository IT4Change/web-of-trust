...
describe('VE-11 Yjs — real gated relay', () => {
  ...
  it('Removal-E2E — admin removes a member + rotates (space-rotate gen+1); the removed member over its OPEN socket is rejected (CAPABILITY_*), its post-rotate write is NOT persisted, and the remaining members do NOT see it; remaining members re-present + converge', async () => {
    ...
    // Admin removes Bob → rotateSpaceKeyAndDistribute + space-rotate(gen+1).
    ...
    // The relay advanced its registered generation (space-rotate landed).
    expect(relay.getSpace(spaceId)?.generation).toBe(genAfter)

    // Bob (removed) attempts a write over its STILL-OPEN socket. Its cached scope is
    // for the OLD generation → the relay rejects it. We assert via the relay log:
    // the count does not grow with a Bob-authored post-rotate entry.
    const countAfterRotate = relay.entryCount(spaceId)
    bobHandle.transact((d) => { d.items['evil'] = { title: 'evil' } })
    await wait(400)

    // MASKING CONTROL: the relay log did NOT grow from Bob's rejected write.
    expect(relay.entryCount(spaceId)).toBe(countAfterRotate)

    // The admin keeps writing post-rotation; convergence holds for the admin.
    adminHandle.transact((d) => { d.items['after'] = { title: 'after' } })
    await wait(250)
    expect(adminHandle.getDoc().items['after']?.title).toBe('after')

    // The admin's plaintext reconstruction does NOT contain Bob's post-rotate write.
    expect(adminHandle.getDoc().items['evil']).toBeUndefined()

    // Positive: removal rotation produced new entries beyond the pre-removal baseline.
