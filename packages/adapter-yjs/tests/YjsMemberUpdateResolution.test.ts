...
describe('Review-M1 — canonical-first: kanonische Bestaetigung VOR dem member-update (Sync 005 Z.194/Z.253)', () => {
  ...
  it('(4) Idempotenz: feuert der Observer-Pfad nach der savePending-Resolution erneut → kein Doppel-Cleanup, kein Fehler', async () => {
    ...
    await (h.adapter as any).resolvePendingMemberUpdates(stateRef)
...
describe('Review-Fix M1 — Resolution-Chain verkettet + Members im Ausfuehrungszeitpunkt (Review-Repro)', () => {
  it('langsame Chain 1 (removed@1) → Chain 2 (active@2) → admin-signiertes Pending removed@3 → KEIN Cleanup, Space bleibt, Pending bleibt offen (Gewinner active@2 < 3)', async () => {
    const h = await setup({ passphrase: 'm1-chain-verkettung' })
    const space = await h.adapter.createSpace<TestDoc>('shared', { items: {} }, { name: 'S' })
    ...
    // Chain 1: kanonisches removed@1 fuer die eigene DID — der Observer-
    // Snapshot dieser Chain ist [ADMIN] (alice entfernt) und damit VERALTET,
    // sobald Chain 2 laeuft.
    applyRemoteMembershipEvent(h.adapter, space.id, { did: h.alice.getDid(), status: 'removed', sinceGeneration: 1 })
