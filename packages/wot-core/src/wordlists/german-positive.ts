 ...
 */
export const germanPositiveWordlist: string[] = [
...
// Validation: Ensure exactly 2048 words
if (germanPositiveWordlist.length !== 2048) {
  throw new Error(
    `German wordlist must contain exactly 2048 words, but has ${germanPositiveWordlist.length}`
  )
}

// Validation: Ensure all first 4 characters are unique
const first4Chars = new Set(germanPositiveWordlist.map((word) => word.slice(0, 4)))
if (first4Chars.size !== 2048) {
  throw new Error(
    `First 4 characters must be unique. Have ${first4Chars.size} unique, need 2048`
  )
}
