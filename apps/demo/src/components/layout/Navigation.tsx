...
export function Navigation() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 md:relative md:border-t-0 md:border-r md:h-screen md:w-64">
      <ul className="grid grid-cols-5 md:flex md:flex-col md:p-4 md:gap-2">
        {navItems.map(({ to, icon: Icon, label }) => (
          <li key={to}>
            <NavLink
              to={to}
              className={({ isActive }) =>
                `flex flex-col md:flex-row items-center gap-1 md:gap-3 py-3 md:px-4 md:py-3 rounded-lg transition-colors ${
                  isActive
            ...
            >
              <Icon size={20} />
              <span className="text-[10px] md:text-sm font-medium leading-tight text-center">{label}</span>
            </NavLink>
          </li>
        ))}
      </ul>
